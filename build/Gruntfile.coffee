...
module.exports = (grunt) ->
  ...
  tmpDir = os.tmpdir()
  appName = if process.platform is 'darwin' then 'WhatsApp.app' else 'WhatsApp'
  ...
  if process.platform is 'win32'
    ...
    killCommand = 'taskkill /F /IM whatsapp.exe'
  else if process.platform is 'darwin'
    contentsDir = path.join(shellAppDir, 'Contents')
    appDir = path.join(contentsDir, 'Resources', 'app')
    installDir ?= path.join('/Applications', appName)
    killCommand = 'pkill -9 WhatsApp'
  else
    contentsDir = shellAppDir
    appDir = path.join(shellAppDir, 'resources', 'app')
  ...
  grunt.initConfig
    ...
    'create-windows-installer':
      ...
      iconUrl: 'https://raw.githubusercontent.com/deviavir/whatsapp-web/master/resources/win/whatsapp.ico'
      setupIcon: path.resolve(__dirname, '..', 'resources', 'win', 'whatsapp.ico')
